...
#ifdef DRAGINO
#include "dragino/DraginoDefaultConfig.h"
#endif
...
#if defined(DRAGINO_REMOTENODE)
static bool draginoCanAdvertiseNodeInfo()
{
    if (privateConfig.hasValidFactoryIdentity() || privateConfig.syncFactoryIdentityFromStorage(false)) {
        return true;
    }

    LOG_WARN("Dragino remote: skip NodeInfo, factory identity missing");
    return false;
}
#endif
...
{
#if defined(DRAGINO_REMOTENODE)
    if (!draginoCanAdvertiseNodeInfo()) {
        return;
    }
#endif
...
meshtastic_MeshPacket *NodeInfoModule::allocReply()
{
    ...
    if (lastSentToMesh && Throttle::isWithinTimespanMs(lastSentToMesh, 15 * 1000)) {
        LOG_DEBUG("Skip send NodeInfo since we sent it <15s ago (gateway mode)");
        ignoreRequest = true;
        return NULL;
    }
#else
...
#if DRAGINO_REMOTE_DISABLE_NODEINFO_AUTO_BROADCAST
    disable();
    LOG_INFO("Dragino remote: NodeInfo auto broadcast disabled");
#else
...
{
#if DRAGINO_REMOTE_DISABLE_NODEINFO_AUTO_BROADCAST
    LOG_INFO("Dragino remote: skip NodeInfo auto broadcast");
    return disable();
#else
