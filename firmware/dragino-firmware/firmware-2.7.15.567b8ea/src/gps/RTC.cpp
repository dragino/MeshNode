...
RTCSetResult perhapsSetRTC(RTCQuality q, const struct timeval *tv, bool forceUpdate)
{
    ...
    static bool hasSetSinceBoot = false;
    uint32_t now = millis();
    uint32_t printableEpoch = tv->tv_sec; // Print lib only supports 32 bit but time_t can be 64 bit on some platforms
#ifdef BUILD_EPOCH
    if (tv->tv_sec < BUILD_EPOCH) {
    ...
    } else if (!hasSetSinceBoot) {
        // First time sync since boot: accept regardless of quality match
        shouldSet = true;
        LOG_DEBUG("First time sync since boot, accepting %s time", RtcName(q));
    } else if (q == RTCQualityGPS) {
        shouldSet = true;
        LOG_DEBUG("Reapply GPS time: %ld secs", printableEpoch);
    } else if (q == RTCQualityNTP && !Throttle::isWithinTimespanMs(lastSetMsec, (1 * 60 * 60 * 1000UL))) {
        // Every 1 hr we will slam in a new NTP or Phone GPS / NTP time, to correct for local RTC clock drift
        shouldSet = true;
        LOG_DEBUG("Reapply external time to correct clock drift %ld secs", printableEpoch);
    } else if (q == RTCQualityFromNet && !Throttle::isWithinTimespanMs(lastSetMsec, (1 * 60 * 60 * 1000UL))) {
        // Every 1 hour allow FromNet time to update, to correct for clock drift on remote nodes
        shouldSet = true;
        LOG_DEBUG("Reapply FromNet time to correct clock drift %ld secs", printableEpoch);
    } else {
        shouldSet = false;
    ...
    if (shouldSet) {
        ...
        // LOG_DEBUG("Synced time to STM32 RTC");
#endif
