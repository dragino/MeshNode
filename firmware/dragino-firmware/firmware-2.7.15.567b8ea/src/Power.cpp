...
#define BATTERY_SENSE_SAMPLES                                                                                                    \
    15 // Set the number of samples, it has an effect of increasing sensitivity in complex electromagnetic environment.
        ...
        if (!initial_read_done || !Throttle::isWithinTimespanMs(last_read_time_ms, min_read_interval)) {
            ...
            LOG_DEBUG("battery gpio %d raw val=%u scaled=%u filtered=%u", BATTERY_PIN, raw, (uint32_t)(scaled), (uint32_t)(last_read_value));
        }
    ...
    virtual bool isBatteryConnect() override
    {
        ...
        for (int i = 2; i < 500; i++) {
            ...
            if (reading != lastReading) {
                return false; // Value changed: USB-powered with no battery connected.
            }
        }
...
bool Power::analogInit()
{
    ...
    LOG_DEBUG("Use analog input %d for battery level", BATTERY_PIN);
        ...
        // The ADC is used to generate a random seed, so we need to initialize it early
        // stm32wlSetup() already configures the pin as analog input, so we don't need to do it again here. 
        LOG_DEBUG("Initializing ADC for random seed generation");
    #else
