...
#define MAX_MQTT_PROXY_MESSAGES 1
...
void MeshService::handleToRadio(meshtastic_MeshPacket &p)
{
    ...
    // if it`s a nodeinfo packet, we want to allow the phone to specify the from address
    ...
    */
    ...
    if (p.decoded.portnum == meshtastic_PortNum_NODEINFO_APP) {
        if (nodeInfoModule) {
            nodeInfoModule->sendOurNodeInfo(NODENUM_BROADCAST, p.decoded.want_response, 0, true);
        }
        return;
    }
#endif
