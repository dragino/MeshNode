...
#if defined(DRAGINO)
#include "dragino/DraginoBuildConfig.h"
#include "dragino/PrivateConfig.h"
#if defined(DRAGINO_REMOTENODE)
#include "dragino/FactoryIdentityManager.h"
#endif
#endif

#if defined(DRAGINO_REMOTENODE) && defined(DRAGINO_STM32)
#include "dragino/DraginoNodeDbSave.h"
#endif
...
NodeDB::NodeDB()
{
...
#if defined(DRAGINO_REMOTENODE)
    bool draginoFactoryKeySynced = false;
#if !MESHTASTIC_EXCLUDE_PKI
    {
        if (privateConfig.hasValidFactoryIdentity() || privateConfig.syncFactoryIdentityFromStorage(true)) {
            draginoFactoryKeySynced = privateConfig.syncSecurityKeyFromFactoryIdentity(true);
            if (!draginoFactoryKeySynced) {
                LOG_WARN("FactoryIdentity: failed to sync security key before PKI init");
            }
        }
    }
#endif
    ...
    (void)draginoFactoryKeySynced;
#endif
#if defined(DRAGINO_REMOTENODE)
    if (!draginoFactoryKeySynced) {
        LOG_WARN("FactoryIdentity: device is missing factory security key");
    }
#endif
...
void NodeDB::installDefaultModuleConfig()
{
...
#if defined(DRAGINO_SERIAL_PROTO_DEFAULT)
    moduleConfig.has_serial = true;
    ...
    moduleConfig.serial.echo = false;
...
#else
    snprintf(owner.long_name, sizeof(owner.long_name), "DGoMesh %04x", getNodeNum() & 0x0ffff);
#endif
...
#ifdef DRAGINO
    if (saveWhat & SEGMENT_PRIVATE_CONFIG) {
        success &= privateConfig.saveAll();
    }
#endif
...
#if defined(DRAGINO_REMOTENODE) && defined(DRAGINO_STM32)
        if (privateConfig.isTrustedTimeSource(nodeId)) {
            dragino::markNodeDbSavePending();
            LOG_DEBUG("Defer trusted NodeDB save until sleep, node=0x%08x", nodeId);
        } else {
            LOG_DEBUG("Skip runtime NodeDB flash save for non-gateway node=0x%08x", nodeId);
        }
#else
...
void NodeDB::set_favorite(bool is_favorite, uint32_t nodeId)
{
    ...
    if (lite && lite->is_favorite != is_favorite) {
        ...
        sortMeshDB();
#if defined(DRAGINO_REMOTENODE) && defined(DRAGINO_STM32)
        LOG_DEBUG("Save favorite NodeDB immediately, node=0x%08x", nodeId);
#endif
        saveNodeDatabaseToDisk();
    }
}
