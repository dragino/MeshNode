...
static void draginoBootTrace(const char *msg)
{
    Serial.begin(115200);
    Serial.print("[BOOT] ");
    Serial.println(msg);
...
void setup()
{
...
#ifdef DEBUG_PORT
    draginoBootTrace("before consoleInit");
    consoleInit(); // Set serial baud rate and init our mesh console
    draginoBootTrace("after consoleInit");
#endif
    ...
    draginoBootTrace("before initDeepSleep");
    initDeepSleep();
    draginoBootTrace("after initDeepSleep");
    ...
    draginoBootTrace("before initSPI");
    initSPI();
    draginoBootTrace("after initSPI");

    draginoBootTrace("before OSThread::setup");
    OSThread::setup();
    draginoBootTrace("after OSThread::setup");
    ...
    draginoBootTrace("before fsInit");
    fsInit();
    draginoBootTrace("after fsInit");

#if defined(EXTERNAL_SENSOR_CONTROL_PIN)
    pinMode(EXTERNAL_SENSOR_CONTROL_PIN, OUTPUT);
    digitalWrite(EXTERNAL_SENSOR_CONTROL_PIN, HIGH);
#if !MESHTASTIC_EXCLUDE_I2C && DRAGINO_SHT3X_ENABLE
#ifdef DRAGINO_SHT3X_POWER_STABILIZE_MS
    delay(DRAGINO_SHT3X_POWER_STABILIZE_MS);
#else
    delay(20);
#endif
    LOG_INFO("SHT3x: sensor power enabled before Meshtastic I2C scan");
#endif
#endif
    ...
    Wire1.begin();
#elif defined(I2C_SDA1) && defined(ARCH_STM32WL)
    Wire1.begin((uint32_t)I2C_SDA1, (uint32_t)I2C_SCL1);
    ...
    Wire.setSCL(I2C_SCL);
    Wire.begin();
#elif defined(I2C_SDA) && defined(ARCH_STM32WL)
...
#if defined(ARCH_STM32WL) && defined(DRAGINO_REMOTENODE) && defined(DRAGINO_STM32)
    draginoBootTrace("before stm32wlSetup");
    stm32wlSetup();
    draginoBootTrace("after stm32wlSetup");
#endif
...
#endif
#endif
    draginoBootTrace("before MeshService");
    service = new MeshService();
    service->init();
    draginoBootTrace("after MeshService");

    // Now that the mesh service is created, create any modules
    draginoBootTrace("before setupModules");
    setupModules();
    draginoBootTrace("after setupModules");
...
// the current region name)
    ...
    draginoBootTrace("before PowerFSM_setup");
    PowerFSM_setup(); // we will transition to ON in a couple of seconds, FIXME, only do this for cold boots, not waking from SDS
    powerFSMthread = new PowerFSMThread();
    draginoBootTrace("after PowerFSM_setup");

#if !HAS_TFT
    setCPUFast(false); // 80MHz is fine for our slow peripherals
#endif

    draginoBootTrace("setup done");
...
{
#if defined(DRAGINO_BOOT_TRACE)
    static bool draginoLoopTraced = false;
    if (!draginoLoopTraced) {
        draginoLoopTraced = true;
        draginoBootTrace("loop enter");
    }
#endif
